...
# =========================
# Joystick Overlay (legacy scripted overlay)
# =========================
class JoystickOverlay:
    """
    Holds smoothed joystick values in [-1,1] and button states.
    Kept for compatibility with scripted modes; 'j' realtime modes do not use this.
    """
    def __init__(self):
...
def run_sequence(stop_event, *, stroke_side=None, tremor_side=None, dysconj_side=None):
    ...
    def R_LR(target):
        ...
        if dysconj_side == 'right': return RIGHT_EE_INWARD_LR if False else RIGHT_EYE_INWARD_LR
        # ↑ Keeps naming consistent while allowing quick toggles if needed.
        return target
...
MODES = {
    "0": ("Joystick control (direct)", run_joystick),
    "1": ("Normal", run_normal),
