...
st.markdown(
    ...
    /* Stop button */
    #btn_stop {
        background-color: var(--duke-red) !important;
        color: white !important;
        border: 2px solid var(--duke-red) !important;
    }
    #btn_stop:hover {
...
def option_button(col, key_char):
    ...
    clicked = col.button(label, use_container_width=True, key=f"btn_{key_char}", help=DESC[key_char], type=btype)
    if clicked:
        send_selection(key_char)
